...
function widget_extended_categories_init ()
{
	...
	function widget_extended_categories ($args, $number = 1)
	{
		...
		if ($avh_extcat_canselectcats) {
			...
			$cat_args = array('include'=>$included_cats, 'orderby'=>$s, 'order'=>$o, 'show_count'=>$c, 'hide_empty'=>$e, 'hierarchical'=>$h, 'title_li'=>'', 'show_option_none'=>__('Selecione o Autor'), 'feed'=>$r, 'feed_image'=>$i, 'name'=>'ec-cat-' . $number, 'depth'=>2);
		} else {
			$cat_args = array('orderby'=>$s, 'order'=>$o, 'show_count'=>$c, 'hide_empty'=>$e, 'hierarchical'=>$h, 'title_li'=>'', 'show_option_none'=>__('Selecione o Autor'), 'feed'=>$r, 'feed_image'=>$i, 'name'=>'ec-cat-' . $number);
		}
		echo $before_widget;
